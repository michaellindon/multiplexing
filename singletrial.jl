...
for iter=1:niter
	...
	for t in Tₚ
		if(rand(Bernoulli((1-Φ(μ+gₚ[t][1]))))==1)
			ξ₀₁₀[t]=Dict{UTF8String,Float64}("Zg"=>rand(Truncated(Normal(μ+gₚ[t][1],1),-Inf,0)), "γ"=>0)
		end
	end
	for t in keys(ξ)
		ξ[t]["Zg"]=rand(Truncated(Normal(μ+g[t][1],1),0,Inf))
	end
	...
	if(log(rand(Uniform(0,1)))<sslogdensity(Zgc,μ,λₚ,qₚ)-sslogdensity(Zgc,μ,λ,q))
		ł=łₚ
		λ=λₚ
		q=qₚ
	end
	g=FFBS(Zgc,collect(keys(Zgc)),μ,σ²,λ,q)
	plot(sort(collect(keys(g))),Λ*Φ([μ+g[key][1] for key in sort(collect(keys(g)))]),c="grey",alpha=0.1);
	g1=[g[i][1] for i in sort(collect(keys(g)))]
	Zgc1=[Zgc[i] for i in sort(collect(keys(Zgc)))]
	n1=length(g1)
	μ=rand(Normal(sum(Zgc1-g1)*(1/((n1/σ²)+(1/σ²ₘ))),sqrt(1/((n1/σ²)+(1/σ²ₘ)))))
	łTrace[iter]=ł
	μTrace[iter]=μ
end
