...
srand(1)
ρ²=1.0; ł=0.2; p=1; d=p+1; ν=p+0.5; 
Tobs=1
g=realization(ł,ρ²,collect(0:Tobs/1000:Tobs))
plot(sort(collect(keys(g))),[g[key][1] for key in sort(collect(keys(g)))],c="green")
Λ=4000; μ=1.0; 
...
for t in Tₚ
	if(rand(Bernoulli(Φ(μ+gₚ[t][1])))==1)
		push!(accepted,t)
	else
		push!(rejected,t)
	end
...
g=copy(gₚ)
plot(sort(collect(keys(g))),[Λ*Φ(μ+g[key][1]) for key in sort(collect(keys(g)))],c="green")
...
for iter=1:niter
	...
	#=logodds=(sslogdensity2(y,1,μ,σ²,ł,ρ²)-sslogdensity2(y,0,μ,σ²,ł,ρ²))=#
	#=odds=exp(logodds)=#
	#=if(odds==Inf)=#
		#=gᵧ=1=#
	#=elseif(odds==-Inf)=#
	...
	#=end=#
	if(gᵧ==1)
		g=FFBS(y,collect(keys(y)),μ,σ²,ł,ρ²)
	else
		g=Dict()
		for key in keys(y)
			g[key]=zeros(4,1)
	...
	łₚ=ł+rand(Normal(0,0.002));
	if(log(rand(Uniform(0,1)))<glogdensity(g,gᵧ,łₚ,ρ²)-glogdensity(g,gᵧ,ł,ρ²))
		ł=łₚ
		accepted=accepted+1
	end
	#=ρ²=rho(g,gᵧ,ł)=#
